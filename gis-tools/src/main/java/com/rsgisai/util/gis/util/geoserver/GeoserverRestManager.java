...
import com.rsgisai.util.gis.util.geoserver.exception.style.GeoserverStyleException;
...
public class GeoserverRestManager {
    ...
    //删除指定 工作区的 指定 图层名称
    public Boolean DeleteLayer(String workspace, String layerName) throws IOException {
        String urlString = String.format("http://www.rsgisai.com:8080/geoserver/rest/workspaces/%s/layers/%s?resurse=false",
                workspace, layerName);
    ...
    public Boolean DeleteDataStores(String workspace, String layerName) throws IOException {
        String urlString = String.format("http://www.rsgisai.com:8080/geoserver/rest/datastores/%s/layers/%s?resurse=false",
                workspace, layerName);
        String authString = username + ":" + password;
        String authStringEnc = Base64.getEncoder().encodeToString(authString.getBytes());
        URL url=new URL(urlString);
